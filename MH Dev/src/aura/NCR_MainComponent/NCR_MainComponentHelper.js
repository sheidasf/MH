({
    ...
    validateContact: function(component, ct, ctnumber, errors) {
        ...
        if (ctnumber == 1) {
            if (!ct.FirstName || !ct.LastName) {
             ...
             errors[errors.length] = 'Last Name is required for Contact # ' + ctnumber + '.\n';
        }             
        ...
        if (ct.FirstName) {
            ...
            if (ct.Is_MLIS_User__c && !ct.Email ) {
                errors[errors.length] = 'Email is required for online users for Contact # ' + ctnumber + '.\n';          
            }
            
            if (ct.Is_MLIS_User__c && !ct.Online_User_Permissions__c) {
                errors[errors.length] = 'Permission needs to be set for online user for Contact # ' + ctnumber + '.\n';
            }
            
            var regex = new RegExp('[0-9]{10}');
            if ( ct.Phone && !regex.test(ct.Phone) ) {
                errors[errors.length] = 'Invalid phone number for Contact # ' + ctnumber + '.\n';          
            }
            if ( ct.MobilePhone && !regex.test(ct.MobilePhone) ) {
                errors[errors.length] = 'Invalid MobilePhone number for Contact # ' + ctnumber + '.\n';          
            }
    ...
    validateHCP: function(component, hcp, hcpNumber, errors) {
        ...
        if (hcpNumber == 1) {
            ...
            errors[errors.length] = 'First Name, Last Name required for Primary Health Care Provider/Authorized Requestor\n';      
            }
        } else if (hcp.FirstName && !hcp.LastName) {
            // first name is a key to know that we need to validate the HCP entered
            errors[errors.length] = 'Last Name required for Health Care Provider/Authorized Requestor ' + hcpNumber + '.\n';           
        }   
               
        // continue with other error checkings.
        if (hcp.FirstName) {
            if ( !hcp.Provider_Credentials__c || hcp.Provider_Credentials__c == '--None--' ) {
                errors[errors.length] = 'Credential is required for Health Care Provider/Authorized Requestor ' + hcpNumber + '.\n';            
            }            
            // NPI and credentials is required for Medical practices
            if (component.get("v.clientAccount.Testing_Purpose__c") == 'Medical') {
                if ( !hcp.NPI_ID__c ) {
                    errors[errors.length] = 'NPI is required for Medical Health Care Provider/Authorized Requestor ' + hcpNumber + '.\n';            
                }
            }            
            if ( hcp.Provider_Credentials__c == 'Other'  && !hcp.Credentials__c) {
                errors[errors.length] = 'Please enter \'Other\' credentials for Health Care Provider/Authorized Requestor ' + hcpNumber + '.\n';             
            }
        }
    ...
    validate : function(component) {    
        ...
        if (signers == 0) {
            errors[errors.length] = 'please select one Health Care Provider/Authorized Requestor as the signer.\n';            
        } else if ( signers > 1) {
            errors[errors.length] = 'Can not select more than one Health Care Provider/Authorized Requestor signers.\n';           
        } 
