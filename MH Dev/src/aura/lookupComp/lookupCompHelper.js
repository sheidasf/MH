({
    ...
    doSearch: function(cmp) {
        ...
        // Mark the action storable, this will help if we hit the server multiple times for the same query
        action.setStorable();
    ...
    loadFirstValue: function(cmp) {
        ...
        var self = this;
        var type = cmp.get("v.sObjectAPIName");
        var value = cmp.get("v.recordId");
        if(!type || !value)
            return;
